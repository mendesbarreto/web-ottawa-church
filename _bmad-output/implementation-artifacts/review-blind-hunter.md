...
Use the `bmad-review-adversarial-general` skill if available.

You get only the diff below. Do not assume product context beyond what the diff proves. Review adversarially for defects, regressions, security/privacy risks, maintainability issues, and incoherent implementation choices.

Return findings only. For each finding include severity, affected file/path, evidence from the diff, and a concrete fix direction.
